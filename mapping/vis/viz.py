...
tmp_pose = None
img_set = []
def draw_trajectory(queue, result_dir, init_pose, cam_scale,
                    save_rendering, near, estimate_c2w_list, gt_c2w_list, fov):
    ...
    if save_rendering:
        os.system(f"rm -rf {result_dir}/mesh_rendering")

    def animation_callback(vis):

        cam = vis.get_view_control().convert_to_pinhole_camera_parameters()
        global traj_pos, tmp_pose
        while True:
            try:
                ...
                if data[0] == 'mesh':
                    ...
                    new_triangles = np.asarray(
                        draw_trajectory.mesh.triangles)
        ...
        if save_rendering:
            ...
            os.makedirs(f'{result_dir}/mesh_rendering', exist_ok=True)
            vis.capture_screen_image(
                f'{result_dir}/mesh_rendering/{draw_trajectory.frame_idx:06d}.jpg')

    vis = o3d.visualization.Visualizer()

    vis.register_animation_callback(animation_callback)
    vis.create_window(window_name=result_dir, height=1080, width=1920)
...
class SLAMFrontend:
    def __init__(self, result_dir, init_pose, cam_scale=1, save_rendering=False,
                 near=0, estimate_c2w_list=None, gt_c2w_list=None, fov=60):
        self.queue = Queue()
        self.p = Process(target=draw_trajectory, args=(
            self.queue, result_dir, init_pose, cam_scale, save_rendering,
            near, estimate_c2w_list, gt_c2w_list, fov))
