...
class SparseVoxelOctreeRayIntersect(Function):
    ...
    def forward(ctx, voxelsize, n_max, points, children, ray_start, ray_dir):
        # HACK: avoid out-of-memory
        S, N = ray_start.shape[:2]
        G = min(int(N / 2048), int(2 * 10 ** 9 / (
                    points.numel() + children.numel())))
        K = int(np.ceil(N / G))
        H = K * G
        if H > N:
            ray_start = torch.cat([ray_start, ray_start[:, : H - N]], 1)
...
class InverseCDFRaySampling(Function):
    ...
    def forward(
        ...
        G, N, P = int(np.ceil(pts_idx.size(0) / 2048)), pts_idx.size(0), pts_idx.size(1)
        # if G == 0:
        #     G = 200
        H = int(np.ceil(N / G)) * G
        ...
        results = [
            _ext.inverse_cdf_sampling(
                pts_idx.contiguous(),
                min_depth.float().contiguous(),
                max_depth.float().contiguous(),
                noise.float().contiguous(),
                probs.float().contiguous(),
                steps.float().contiguous(),
                fixed_step_size,
            )
        ]
        #
...
def ray_sample(intersection_outputs, step_size=0.01, fixed=False):
    ...
    sampled_idx, sampled_depth = inverse_cdf_sampling(
